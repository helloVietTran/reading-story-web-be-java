...
public class InventoryService {
    ...
    public InventoryResponse getUserInventory(){
        ...
        Optional<Inventory> userInventoryOptional = inventoryRepository.findFirstByUserIdAndExpirationDateAfter(userId, Instant.now());

        if(userInventoryOptional.isEmpty())
            throw new AppException(ErrorCode.USER_INVENTORY_NOT_EXISTED);
